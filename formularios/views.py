...
from .models import Categoria, Formulario, FormularioIndexVersion, Pagina
from django.shortcuts import get_object_or_404
from .serializers import CategoriaSerializer, CrearCampoEnPaginaSerializer, FormularioSerializer
...
class PaginaViewSet(viewsets.ViewSet):
    @action(detail=True, methods=["post"], url_path="campos")
    def crear_campo(self, request, pk=None):
        id_pagina = _uuid32_no_dashes(pk)  # <-- normaliza aquí también
        ser = CrearCampoEnPaginaSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        out = crear_campo_en_pagina(id_pagina, ser.validated_data)
...
#     def crear_campo_actual(self, request, pk=None):
#         """
#         Crea un Campo publicando una NUEVA versión del formulario.
#         'pk' es SIEMPRE id_pagina (estable).
#         """
#         pagina = get_object_or_404(Pagina, pk=pk)
#         payload = request.data.copy()
#         # Validamos el schema del Campo, pero NO dejamos que cambien la página
#         ser = CampoSerializer(data=payload, context=self.get_serializer_context())
#         ser.is_valid(raise_exception=True)

#         ver_new, pagina_estable, campo = publicar_nueva_version_por_cambio_en_pagina(
#             pagina=pagina,
...
#             "pagina_id": str(pagina_estable.id_pagina),     # 👈 MISMO id
